...
var palladioSources = [ "palladio-table-view.js" ];
var palladioCSS = [ "palladio-table-view.css" ];
...
gulp.task('scripts', function () {
	...
	var templates = gulp.src(palladioTemplate)
		.pipe(angularTemplates({ module: 'palladio', basePath: 'partials/palladio-table-component/' }))
		.pipe(rename('templates.tmpl'));

	merge(files, templates)
		.pipe(order(['*.js', '*.tmpl']))
        .pipe(concat('palladio-table-component.js'))
        .pipe(gulp.dest('./dist/'))
        .pipe(uglify())
        .pipe(rename('palladio-table-component.min.js'))
        .pipe(gulp.dest('./dist/'));
});

gulp.task('css', function () {
	gulp.src(palladioCSS)
		.pipe(concat('palladio-table-component.css'))
		.pipe(gulp.dest('./dist/'));
});
